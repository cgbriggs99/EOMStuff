...
void GeneralCC::tei_ao_to_antisym_so(std::shared_ptr<MintsHelper> mintshelper) {
    if (integral_type_ == FULL) {
        ...
        int naux = integrals_.df->naux;

        {
            auto B_a_Qab = tei_method.contract_B(Ca_virt, Ca_virt);
            auto B_b_Qab = tei_method.contract_B(Cb_virt, Cb_virt);

            integrals_.df->B_Qab = std::make_shared<Tensor<double, 3>>("B_Qab", naux, nvirt_, nvirt_);
            integrals_.df->B_Qab->zero();
            ...
            for (int a = 0; a < nvirt_; a++) {
                ...
                for (int b = 0; b < nvirt_; b++) {
                    ...
                    if (spin_a == spin_b) {
                        if (spin_a) {
                            integrals_.df->B_Qab->subscript(All, a, b) = B_a_Qab(All, a, b);
                        } else {
                            integrals_.df->B_Qab->subscript(All, a, b) = B_b_Qab(All, a - navirt_, b - navirt_);
                        }
                    }
                }
            }
...

        {
            ...
            integrals_.df->B_Qai = std::make_shared<Tensor<double, 3>>("B_Qai", nvirt_, nocc_, naux);
            integrals_.df->B_Qai->zero();
            ...
            for (int a = 0; a < nvirt_; a++) {
                ...
                for (int i = 0; i < nocc_; i++) {
                    ...
                    if (spin_a == spin_i) {
                        if (spin_a) {
                            integrals_.df->B_Qai->subscript(All, a, i) = B_a_Qai(All, a, i);
                        } else {
                            integrals_.df->B_Qai->subscript(All, a, i) = B_b_Qai(All, a - navirt_, i - nalpha());
                        }
                    }
                }
            }
...

        {
            ...
            integrals_.df->B_Qia = std::make_shared<Tensor<double, 3>>("B_Qia", nocc_, nvirt_, naux);
            integrals_.df->B_Qia->zero();
            ...
            for (int i = 0; i < nocc_; i++) {
                ...
                for (int a = 0; a < nvirt_; a++) {
                    ...
                    if (spin_i == spin_a) {
                        if (spin_i) {
                            integrals_.df->B_Qia->subscript(All, i, a) = B_a_Qia(All, i, a);
                        } else {
                            integrals_.df->B_Qia->subscript(All, i, a) = B_b_Qia(All, i - nalpha(), a - navirt_);
                        }
                    }
                }
            }
...

        {
            ...
            integrals_.df->B_Qij = std::make_shared<Tensor<double, 3>>("B_Qij", nocc_, nvirt_, naux);
            integrals_.df->B_Qij->zero();
            ...
            for (int i = 0; i < nocc_; i++) {
                ...
                for (int j = 0; j < nocc_; j++) {
                    ...
                    if (spin_i == spin_j) {
                        if (spin_i) {
                            integrals_.df->B_Qij->subscript(All, i, j) = B_a_Qij(All, i, j);
                        } else {
                            integrals_.df->B_Qij->subscript(All, i, j) = B_b_Qij(All, i - nalpha(), j - nalpha());
                        }
                    }
                }
            }
...
#pragma omp parallel for collapse(2)
        ...
        // <ai||jb>
        // Contract with non-transformed <ai||eb>
        for (int e = 0; e < nvirt_; e++) {
            auto t1_view = (*t1_amps_)(e, All);
            for (int b = 0; b < nvirt_; b++) {
                if (vv_mults(e, b) == 0) {
                ...
                auto W_view = (*integrals_.full->W_a_x_bc_i)(All, vv_inds(e, b), All);
                auto out_view = (*t1_integrals_.full->W_a_j_b_i)(All, All, b, All);
                tensor_algebra::einsum(1.0, Indices { index::a, index::j, index::i }, &out_view, (double) vv_mults(e, b), Indices {
                        index::a, index::i }, W_view, Indices { index::j }, t1_view);
            }
        }

        // Contract with transformed -<mi||jb>
        ...
        for (int m = 0; m < nocc_; m++) {
            ...
            for (int i = 0; i < nocc_; i++) {
                ...
                auto W_view = (*t1_integrals_.full->W_x_k_a_ij)(All, All, oo_inds(m, i));
                auto out_view = (*t1_integrals_.full->W_a_j_b_i)(All, All, All, i);

                tensor_algebra::einsum(1.0, Indices { index::a, index::j, index::b }, &out_view, (double) -oo_mults(m, i), Indices {
                        index::j, index::b }, W_view, Indices { index::a }, t1_view);
            }
        }
    } else {
        if (t1_integrals_.df == nullptr) {
            t1_integrals_.df = new detail::DFIntegrals();

            t1_integrals_.df->naux = integrals_.df->naux;

            t1_integrals_.df->B_Qia = integrals_.df->B_Qia; // These two are always equal, so they can alias each other.

            t1_integrals_.df->B_Qab = std::make_shared<Tensor<double, 3>>("t1 B_Qab", t1_integrals_.df->naux, nvirt_, nvirt_);
            t1_integrals_.df->B_Qai = std::make_shared<Tensor<double, 3>>("t1 B_Qai", t1_integrals_.df->naux, nvirt_, nocc_);
            t1_integrals_.df->B_Qij = std::make_shared<Tensor<double, 3>>("t1 B_Qij", t1_integrals_.df->naux, nocc_, nocc_);
        }

        Tensor<double, 1> tbQke { "tke bQke", integrals_.df->naux };
        Tensor<double, 2> tbQie { "tke bQie", nocc_, nocc_ }, tbQae { "tke bQae", nvirt_, nocc_ };

        Tensor<double, 2> t1_sort { "t1 sort", nocc_, nvirt_ };

        // Sort the t1 integrals.
        tensor_algebra::permute(Indices { index::i, index::a }, &t1_sort, Indices { index::a, index::i }, *t1_amps_);

        // Compute the first intermediate.
        tensor_algebra::einsum(Indices { index::Q }, &tbQke, Indices { index::Q, index::k, index::e }, *integrals_.df->B_Qia, Indices {
                index::k, index::e }, t1_sort);

        // Fia term 2
        tensor_algebra::einsum(1.0, Indices { index::i, index::a }, t1_F_ia_.get(), 1.0, Indices { index::Q, index::i, index::a },
                *integrals_.df->B_Qia, Indices { index::Q }, tbQke);

        // Fij term 3
        tensor_algebra::einsum(1.0, Indices { index::i, index::j }, t1_F_ij_.get(), 1.0, Indices { index::Q, index::index::i, index::j },
                *integrals_.df->B_Qij, Indices { index::Q }, tbQke);

        // Fab term 3
        tensor_algebra::einsum(1.0, Indices { index::a, index::b }, t1_F_ab_.get(), 1.0, Indices { index::Q, index::a, index::b },
                *integrals_.df->B_Qab, Indices { index::Q }, tbQke);

        // Fai term 4
        tensor_algebra::einsum(1.0, Indices { index::a, index::i }, t1_F_ai_.get(), 1.0, Indices { index::Q, index::a, index::i },
                *integrals_.df->B_Qai, Indices { index::Q }, tbQke);

        for (int Q = 0; Q < integrals_.df->naux; Q++) {
            auto B_ia_view = (*integrals_.df->B_Qia)(Q, All, All);
            auto B_ij_view = (*integrals_.df->B_Qij)(Q, All, All);
            auto B_ab_view = (*integrals_.df->B_Qab)(Q, All, All);
            auto B_ai_view = (*integrals_.df->B_Qai)(Q, All, All);
            // Compute the second intermediate.
            tensor_algebra::einsum(Indices { index::i, index::j }, &tbQie, Indices { index::i, index::e }, B_ia_view, Indices { index::e,
                    index::j }, *t1_amps_);
            // Compute the third intermediate.
            tensor_algebra::einsum(Indices { index::a, index::i }, &tbQae, Indices { index::a, index::e }, B_ab_view, Indices { index::e,
                    index::i }, *t1_amps_);

            // Fia term 3
            tensor_algebra::einsum(1.0, Indices { index::i, index::a }, t1_F_ia_.get(), -1.0, Indices { index::k, index::a }, B_ia_view,
                    Indices { index::i, index::k }, tbQie);

            // Fij term 4
            tensor_algebra::einsum(1.0, Indices { index::i, index::j }, t1_F_ij_.get(), -1.0, Indices { index::k, index::j }, B_ij_view,
                    Indices { index::i, index::k }, tbQie);

            // Fab term 4
            tensor_algebra::einsum(1.0, Indices { index::a, index::b }, t1_F_ab_.get(), -1.0, Indices { index::k, index::b }, B_ia_view,
                    Indices { index::a, index::k }, tbQae);

            // Fai term 5
            tensor_algebra::einsum(1.0, Indices { index::a, index::i }, t1_F_ai_.get(), -1.0, Indices { index::k, index::i }, B_ij_view,
                    Indices { index::a, index::k }, tbQae);
        }
        ...
        // Fij term 2
        tensor_algebra::einsum(1.0, Idnices { index::i, index::j }, t1_F_ij_.get(), 1.0, Indices { index::i, index::e }, *t1_F_ia_,
                Indices { index::e, index::j }, *t1_amps_);
        // Fij is done
        ...
        // Fai term 3
        auto &temp_oo = tbQie;        // Reuse temporary storage.

        temp_oo = *t1_F_ij_;

        tensor_algebra::einsum(1.0, Indices { index::m, index::i }, &temp_oo, -1.0, Indices { index::m, index::e }, *t1_F_ia_, Indices {
                index::e, index::i }, *t1_amps_);

        tensor_algebra::einsum(1.0, Indices { index::a, index::i }, t1_F_ai_.get(), -1.0, Indices { index::m, index::i }, temp_oo, Indices {
                index::a, index::m }, *t1_amps_);
        // Fai is done.

        // Two-electron terms.
        // Skip bQia since it never changes.

        // bQij
        *t1_integrals_.df->B_Qij = *integrals_.df->B_Qij; // term 1

        // term 2
        tensor_algebra::einsum(1.0, Indices { index::Q, index::i, index::j }, t1_integrals_.df->B_Qij.get(), 1.0, Indices { index::Q,
                index::i, index::e }, *t1_integrals_.df->B_Qia, Indices { index::e, index::j }, *t1_amps_);

        // bQab
        *t1_integrals_.df->B_Qab = *integrals_.df->B_Qab; // term 1

        // term 2
        tensor_algebra::einsum(1.0, Indices { index::Q, index::a, index::b }, t1_integrals_.df->B_Qab.get(), -1.0, Indices { index::Q,
                index::m, index::b }, *t1_integrals_.df->B_Qia, Indices { index::a, index::m }, *t1_amps_);

        // bQai
        *t1_integrals_.df->B_Qai = *integrals_.df->B_Qai; // term 1

        // term 2
        tensor_algebra::einsum(1.0, Indices { index::Q, index::a, index::i }, t1_integrals_.df->B_Qai.get(), -1.0, Indices { index::Q,
                index::m, index::i }, *t1_integrals_.df->B_Qij, Indices { index::a, index::m }, *t1_amps_);

        // term 3
        tensor_algebra::einsum(1.0, Indices { index::Q, index::a, index::i }, t1_integrals_.df->B_Qai.get(), 1.0, Indices { index::Q,
                index::a, index::e }, *integrals_.df->B_Qab, Indices { index::e, index::i }, *t1_amps_);
    }
}
}
